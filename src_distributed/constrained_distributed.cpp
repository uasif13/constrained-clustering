#include "constrained_distributed.h"

void ConstrainedClusteringDistributed::WriteClusterQueue(std::queue<std::vector<int>>& cluster_queue, igraph_t* graph) {
...
void ConstrainedClusteringDistributed::WritePartitionMap(std::map<int, int>& final_partition) {
    std::ofstream clustering_output(this->output_file);
...
int ConstrainedClusteringDistributed::WriteToLogFile(std::string message, Log message_type) {
