...
int main(int argc, char* argv[]) {
    ...
    argparse::ArgumentParser mpi("MPI")
    mpi.add_description("MPI")
    ...
    cm.add_argument("--nprocs")
        .require()
        .deafult_value(int(1))
        .help("Number of MPI Processes")
        .scan<'d', int>();
    mincut_only.add_argument("--mpi_nprocs")
        .require()
        .default_value(int(1))
        .help("Number of MPI Processes")
        .scan<'d', int>();
    ...
    if(main_program.is_subcommand_used(cm)) {
        ...
        int nprocs = cm.get<int>("nprocs");
        ConstrainedClustering* cm = new CM(edgelist, algorithm, resolution, existing_clustering, num_processors, mpi_nprocs, output_file, log_file, log_level);
        ...
        ConstrainedClustering* mincut_only = new MincutOnly(edgelist, existing_clustering, num_processors, mpi_nprocs, output_file, log_file, connectedness_criterion, log_level);
        random_functions::setSeed(0);
        mincut_only->main();
        delete mincut_only;
    }
