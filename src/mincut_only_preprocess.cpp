...
/* G(n) = C n^x, where C and x are parameters specified by the user (here, presumably 0<x<2). Note that x=1 makes it linear. */
    ...
    this->WriteToLogFile("Finished Getting Connected Components size: " + std::to_string(subgraph_edges_vector.size()) , Log::info);
    printf("Finished Getting Connected Components size: %d\n", subgraph_edges_vector.size());
    ...
    if(iter_count % 10000 == 0) {
        ...
        this->WriteToLogFile(std::to_string(subgraph_edges_vector.size()) + " [connected components / clusters] to be mincut", Log::info);
    }
