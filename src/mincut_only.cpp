...
/* G(n) = C n^x, where C and x are parameters specified by the user (here, presumably 0<x<2). Note that x=1 makes it linear. */
    ...
    printf("Original to New Id Map edgelist size: %d\n",original_to_new_id_map.size());
    ...
    printf("Finished reading communities size: %d\n",new_node_id_to_cluster_id_map.size());
    ...
    connected_components_vector = ConstrainedClustering::GetConnectedComponents(&graph);    
