...
/* G(n) = C n^x, where C and x are parameters specified by the user (here, presumably 0<x<2). Note that x=1 makes it linear. */
    ...
    std::vector<std::vector<long>> connected_components_vector = MMapSubgraphLoader::LoadEdgelistMMap(this -> edgelist, &original_to_new_id_unordered_map, false);
    ...
    if(current_connectedness_criterion == ConnectednessCriterion::Simple) {
        ...
        while (true) {
            ...
            if(before_mincut_number_of_clusters > 1) {
                ...
                for(int i = 0; i < this->num_processors; i ++) {
                    thread_vector.push_back(std::thread(MincutOnly::MinCutWorker, current_connectedness_criterion, connectedness_criterion_c, connectedness_criterion_x, pre_computed_log, this->mincut_type));
                }
                ...
                MincutOnly::to_be_mincut_clusters.push({-1});
                MincutOnly::MinCutWorker(current_connectedness_criterion, connectedness_criterion_c, connectedness_criterion_x, pre_computed_log);
            }
