...
class CMPreprocess : public ConstrainedClustering {
    public:
        CMPreprocess(std::string edgelist, std::string algorithm, double clustering_parameter, int num_processors, std::string output_file, std::string log_file, int log_level, int my_rank, int nprocs) : ConstrainedClustering(edgelist, algorithm, clustering_parameter, "", num_processors, output_file, log_file, log_level, my_rank, nprocs) {
        };
        ...
        static inline void MinCutOrClusterWorkerRecursive(vector<long> current_cluster_edges, std::string algorithm, int seed, double clustering_parameter) {
            ...
            igraph_vit_t vit;
            igraph_integer_t subgraph_size;
                ...
                if(is_well_connected) {
                    ...
                    if(in_partition.size() > 1) {
                        ...
                        for(size_t i = 0; i < in_clusters.size(); i ++) {
                            ...
                            // }
                            {
                                CMPreprocess::MinCutOrClusterWorkerRecursive(in_clusters[i], algorithm, seed, clustering_parameter);
                            }
                        }
                    }
                    if(out_partition.size() > 1) {
                        ...
                        for(size_t i = 0; i < out_clusters.size(); i ++) {
                            ...
                            // }
                            {
                                CMPreprocess::MinCutOrClusterWorkerRecursive(out_clusters[i], algorithm, seed, clustering_parameter);
                            }
                        }
                    }
                }
