...
class CMPreprocess : public ConstrainedClustering {
    public:
        ...
        static inline void MinCutOrClusterWorkerRecursive(vector<long> current_cluster, std::string algorithm, int seed, double clustering_parameter) {
            ...
            for (int i = 0; i < current_cluster.size(); i++) {
                VECTOR(edges)[i] = current_cluster[i];
            }
            igraph_t subgraph;
            ...
            for(long i = 0; i < igraph_vcount(&subgraph); i ++) {
                new_id_to_old_id_map[i] = stol(VAS(&subgraph, "name", i));
                old_id_to_new_id_map[stol(VAS(&subgraph, "name", i))] = i;
                std::cout << i <<  " " << VAS(&subgraph, "name", i) << std::endl;
            }
            ...
            while(!(is_non_trivial_cut || is_well_connected)) {
                ...
                if(is_well_connected) {
                    ...
                    if(in_partition.size() > 1) {
                        std::vector<std::vector<long>> in_clusters = CMPreprocess::RunClusterOnPartition(&subgraph, algorithm, seed, clustering_parameter, in_partition);
                        for(size_t i = 0; i < in_clusters.size(); i ++) {
                            ...
                            {
                                CMPreprocess::MinCutOrClusterWorkerRecursive(translated_in_clusters, algorithm, seed, clustering_parameter);
                            }
                        }
                    }
                    if(out_partition.size() > 1) {
                        std::vector<std::vector<long>> out_clusters = CMPreprocess::RunClusterOnPartition(&subgraph, algorithm, seed, clustering_parameter, out_partition);
                        for(size_t i = 0; i < out_clusters.size(); i ++) {
                            ...
                            {
                                CMPreprocess::MinCutOrClusterWorkerRecursive(translated_out_clusters, algorithm, seed, clustering_parameter);
                            }
                        }
                    }
            }
