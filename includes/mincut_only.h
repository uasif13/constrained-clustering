...
class MincutOnly : public ConstrainedClustering {
    public:
        ...
        static inline std::vector<std::vector<long long>> GetConnectedComponentsOnPartition(const igraph_t* graph, std::vector<long long>& partition) {
        ...
        static inline void MinCutWorker(igraph_t* graph, ConnectednessCriterion connectedness_criterion) {
            while (true) {
                ...
                std::vector<long long> current_cluster = MincutOnly::to_be_mincut_clusters.front();
                ...
                std::vector<long long> in_partition = mcc.GetInPartition();
                std::vector<long long> out_partition = mcc.GetOutPartition();
                ...
                if(!current_criterion) {
                    ...
                    if(in_partition.size() > 1) {
                        std::vector<std::vector<long long>> in_clusters = GetConnectedComponentsOnPartition(&induced_subgraph, in_partition);
                    ...
                    if(out_partition.size() > 1) {
                        std::vector<std::vector<long long>> out_clusters = GetConnectedComponentsOnPartition(&induced_subgraph, out_partition);
    ...
    private:
        ...
        static inline std::queue<std::vector<long long>> done_being_mincut_clusters;
