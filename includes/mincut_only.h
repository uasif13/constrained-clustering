...
class MincutOnly : public ConstrainedClustering {
    public:
        ...
        static inline std::vector<std::vector<long>> GetConnectedComponentsOnPartition(const igraph_t* graph, std::vector<int>& partition, std::unordered_map<long, long> prev_new_id_to_old_id_map) {
        ...
        static inline void MinCutWorker( ConnectednessCriterion current_connectedness_criterion, double connectedness_criterion_c, double connectedness_criterion_x, double pre_computed_log, std::string mincut_type = "cactus") {
            while (true) {
                ...
                MinCutCustom mcc(&subgraph, mincut_type);
                ...
                if(!current_criterion) {
                        ...
                        for(size_t i = 0; i < in_clusters.size(); i ++) {
                            {
                                ...
                                MincutOnly::to_be_mincut_clusters.push(in_clusters[i]);
                            }
                        }
                    }
                    if(out_partition.size() > 1) {
                        ...
                        for(size_t i = 0; i < out_clusters.size(); i ++) {
                            {
                                std::lock_guard<std::mutex> to_be_mincut_guard(MincutOnly::to_be_mincut_mutex);
                                MincutOnly::to_be_mincut_clusters.push(out_clusters[i]);
                            }
                        }
                    }
                } else {
                    ...
                    MincutOnly::done_being_mincut_clusters.push(cc);
