...
class ConstrainedClustering {
    public:
        ...
        void WritePartitionMap(std::map<long long,long long>& final_partition);
        void WriteClusterQueue(std::queue<std::vector<long long>>& to_be_clustered_clusters, igraph_t* graph);
        long long WriteClusterQueueMPI(std::queue<std::vector<long long>>* cluster_queue, igraph_t* graph, long long cluster_start, long long previous_cluster_id, int iteration, uint64_t* opCount);

        int initializeSlice(igraph_t * graph){
            this -> vertex_count = igraph_vcount(graph);
            long long my_work = vertex_count/nprocs;
            ...
            for(; !IGRAPH_EIT_END(eit); IGRAPH_EIT_NEXT(eit)) {
                ...
                long long from_node = IGRAPH_FROM(graph, current_edge);
                long long to_node = IGRAPH_TO(graph, current_edge);
        ...
        static inline std::map<std::string, long long> GetOriginalToNewIdMap(igraph_t* graph) {
            std::map<std::string, long long> original_to_new_id_map;
            igraph_vit_t vit;
        ...
        std::map<std::string, long long> GetOriginalToNewIdMapDistributed(igraph_t* graph) {
            std::map<std::string, long long> original_to_new_id_map;
            ...
            for(;!IGRAPH_VIT_END(vertex_iter); IGRAPH_VIT_NEXT(vertex_iter)) {
                long long node_id = IGRAPH_VIT_GET(vertex_iter);
                original_to_new_id_map[VAS(graph, "name", node_id)] = node_id;
            }
            return original_to_new_id_map;
        }

        static inline std::map<long long, long long> ReadCommunities(const std::map<std::string, long long>& original_to_new_id_map, std::string existing_clustering) {
            std::map<long long, long long> partition_map;
            std::ifstream existing_clustering_file(existing_clustering);
            std::string node_id;
            long long cluster_id = -1;
            while (existing_clustering_file >> node_id >> cluster_id) {
                if (original_to_new_id_map.contains(node_id)) {
                    long long new_node_id = original_to_new_id_map.at(node_id);
        ...
        static inline std::map<long long, long long> ReadCommunities(std::string existing_clustering) {
            std::map<long long, long long> partition_map;
            std::ifstream existing_clustering_file(existing_clustering);
            long long node_id = -1;
            long long cluster_id = -1;
        ...
        std::map<long long, long long> ReadCommunitiesDistributed(std::string existing_clustering) {
            std::map<long long, long long> partition_map;
            std::ifstream existing_clustering_file(existing_clustering);
            long long node_id = -1;
            long long cluster_id = -1;
        ...
        static inline void ClusterQueueToMap(std::queue<std::vector<long long>>& cluster_queue, std::map<long long, long long>& node_id_to_cluster_id_map) {
            long long cluster_id = 0;
        ...
        static inline void RemoveInterClusterEdges(igraph_t* graph, const std::map<long long,long long>& node_id_to_cluster_id_map) {
        ...
        static inline void RemoveInterClusterEdges(igraph_t* graph, const std::unordered_map<long long,long long>& node_id_to_cluster_id_map, int num_threads) {
            const long long num_nodes = igraph_vcount(graph);
            const long long total_edges = igraph_ecount(graph);
            
            std::vector<long long> node_to_cluster(num_nodes, -1);
            ...
            long long thread_edges_size = total_edges / num_threads / 2;
            
            std::vector<std::vector<long long>> thread_local_edges(num_threads);
            for (int i = 0; i < num_threads; i++) {
                thread_local_edges[i].reserve(thread_edges_size);
            }

            auto filter_edges = [&](long long thread_id, long long start_edge, long long end_edge) {
                igraph_eit_t eit;
                igraph_es_t es;
                ...
                for (; !IGRAPH_EIT_END(eit); IGRAPH_EIT_NEXT(eit)) {
                    ...
                    long long from_node = IGRAPH_FROM(graph, edge_id);
                    long long to_node = IGRAPH_TO(graph, edge_id);

                    long long from_cluster = node_to_cluster[from_node];
                    long long to_cluster = node_to_cluster[to_node];
            ...
            const long long chunk_size = (total_edges + num_threads - 1) / num_threads;

            for (int i = 0; i < num_threads; i++) {
                long long start = static_cast<long long>(i) * chunk_size;
                long long end = std::min(start + chunk_size, total_edges);
                if (start < end) {
        ...
        // currently keeps only those edges that go from within these clusters defined in the map
        static inline igraph_vector_int_t RemoveInterClusterEdgesDistributed(igraph_t* graph, const std::map<long long,long long>& node_id_to_cluster_id_map) {
        ...
        static inline void RestoreInterClusterEdges(const igraph_t* original_graph, igraph_t* graph, const std::map<long long,long long>& node_id_to_cluster_id_map) {
            struct ClusterEdgeStruct {
                long long cluster_id;
                long long other_cluster_id;
            ...
            std::map<long long, std::vector<long long>> cluster_id_to_node_vec_map;
            std::map<long long, std::set<long long>> cluster_id_to_node_set_map;
            std::map<long long, long long> cluster_id_to_num_edges_map;
            std::map<long long, std::map<long long, long long>> cluster_id_to_cluster_id_to_num_edges_map;
            std::priority_queue<ClusterEdgeStruct> pq;
            std::map<long long, std::vector<long long>> clusters_to_be_merged;
            std::set<long long> clusters_already_chosen_to_be_merged;
            ...
            for(; !IGRAPH_EIT_END(eit); IGRAPH_EIT_NEXT(eit)) {
                igraph_integer_t current_edge = IGRAPH_EIT_GET(eit);
                long long from_node = IGRAPH_FROM(original_graph, current_edge);
                long long to_node = IGRAPH_TO(original_graph, current_edge);
            ...
            for(auto const& [cluster_id, other_cluster_id_to_num_edges_map]: cluster_id_to_cluster_id_to_num_edges_map) {
                for(auto const& [other_cluster_id, num_edges_from_cluster_id_to_other_cluster_id]: other_cluster_id_to_num_edges_map) {
                    long long current_cluster_id_num_edges = cluster_id_to_num_edges_map[cluster_id];
                    long long other_cluster_id_num_edges = cluster_id_to_num_edges_map[other_cluster_id];
            ...
            while(!pq.empty()) {
                ...
                pq.pop();
                long long cluster_id = current_cluster_edge_struct.cluster_id;
                long long other_cluster_id = current_cluster_edge_struct.other_cluster_id;
        ...
        static inline void RestoreInterClusterEdges(const igraph_t* original_graph, igraph_t* graph, const std::map<long long,long long>& node_id_to_cluster_id_map, long long cluster_id, long long other_cluster_id) {
            igraph_eit_t eit;
            igraph_eit_create(original_graph, igraph_ess_all(IGRAPH_EDGEORDER_ID), &eit);
            for(; !IGRAPH_EIT_END(eit); IGRAPH_EIT_NEXT(eit)) {
                igraph_integer_t current_edge = IGRAPH_EIT_GET(eit);
        ...
        static inline void RunLouvainAndUpdatePartition(std::map<long long, long long>& partition_map, int seed, igraph_t* graph) {
        ...
        static inline void RunLeidenAndUpdatePartition(std::map<long long, long long>& partition_map, MutableVertexPartition* partition, int seed, igraph_t* graph, int num_iter= 2) {
            Optimiser o;
            o.set_rng_seed(seed);
        ...
        static inline std::map<long long, long long> GetCommunities(std::string edgelist, std::string algorithm, int seed, double clustering_parameter, igraph_t* graph_ptr) {
            std::map<long long, long long> partition_map;
            igraph_t graph;
            if(graph_ptr == NULL) {
        ...
        static inline std::vector<std::vector<long long>> GetConnectedComponents(igraph_t* graph_ptr) {
            std::vector<std::vector<long long>> connected_components_vector;
            std::map<long long, std::vector<long long>> component_id_to_member_vector_map;
            ...
            for(long long node_id = 0; node_id < igraph_vcount(graph_ptr); node_id ++) {
                long long current_component_id = VECTOR(component_id_vector)[node_id];
        ...
        std::vector<std::vector<long long>> GetConnectedComponentsDistributed(igraph_t* graph_ptr, const std::unordered_map<long long,long long>& node_id_to_cluster_id_map, int my_rank, int nprocs) {
            std::vector<std::vector<long long>> connected_components_vector;
            std::unordered_map<long long, std::vector<long long>> component_id_to_member_vector_map;
            igraph_vector_int_t component_id_vector;
            igraph_vector_int_init(&component_id_vector, 0);
            igraph_vector_int_t membership_size_vector;
            igraph_vector_int_init(&membership_size_vector, 0);
            ...
            long long no_of_vertices = igraph_vcount(graph_ptr);
            igraph_vector_int_resize(&component_id_vector, no_of_vertices);
            for (long long vertex = 0; vertex < no_of_vertices; ++vertex) {
            ...
            std::vector<long long> node_to_original_id(igraph_vcount(graph_ptr));
            // for(long long i = 0; i < igraph_vcount(graph_ptr); i++) {
            ...
            for (long long vertex = 0; vertex < no_of_vertices; ++vertex) {
                ...
                while (!igraph_dqueue_int_empty(&q)) {
                    long long act_node = igraph_dqueue_int_pop(&q);
                    // printf("act_node inside queue: %ld\n", act_node);

                    igraph_neighbors(graph_ptr, &neis, act_node, IGRAPH_ALL, IGRAPH_NO_LOOPS, IGRAPH_MULTIPLE);
                    long long nei_count = igraph_vector_int_size(&neis);
                    for (long long i = 0; i < nei_count; i++) {
                        long long neighbor = VECTOR(neis)[i];
            ...
            for (long long node_id = 0; node_id < no_of_vertices; node_id++) {
        ...
        static inline bool IsWellConnected(long long in_partition_size, long long out_partition_size, int edge_cut_size) {
            bool node_connectivity = log10(in_partition_size + out_partition_size) < edge_cut_size;
            return node_connectivity;
        }

        static inline bool IsWellConnected(const std::vector<long long>& in_partition, const std::vector<long long>& out_partition, int edge_cut_size, const igraph_t* induced_subgraph) {
            if(edge_cut_size == 0) {
                return false;
            }
    ...
    protected:
        ...
        set<long long> vertices_in_slice;
        igraph_vector_int_t vertex_vec;
        set<long long> vertex_set;
};
