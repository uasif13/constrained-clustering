...
class CM : public ConstrainedClustering {
    public:
        ...
        static inline std::vector<std::vector<long long>> RunClusterOnPartition(const igraph_t* graph, std::string algorithm, int seed, double clustering_parameter, std::vector<long long>& partition) {
            std::vector<std::vector<long long>> cluster_vectors;
            std::map<long long, std::vector<long long>> cluster_map;
            ...
            std::map<long long, long long> partition_map = ConstrainedClustering::GetCommunities("", algorithm, seed, clustering_parameter, &induced_subgraph);
            ConstrainedClustering::RemoveInterClusterEdges(&induced_subgraph, partition_map);
            std::vector<std::vector<long long>> connected_components_vector = ConstrainedClustering::GetConnectedComponents(&induced_subgraph);
            for(size_t i = 0; i < connected_components_vector.size(); i ++) {
                std::vector<long long> translated_cluster_vector;
                for(size_t j = 0; j < connected_components_vector.at(i).size(); j ++) {
                    long long new_id = connected_components_vector.at(i).at(j);
        ...
        static inline void MinCutOrClusterWorker(const igraph_t* graph, std::string algorithm, int seed, double clustering_parameter) {
            while (true) {
                ...
                std::vector<long long> current_cluster = CM::to_be_mincut_clusters.front();
                std::set<long long> current_cluster_set(current_cluster.begin(), current_cluster.end());
                ...
                std::map<long long, long long> new_id_to_old_id_map;
                std::map<long long, long long> newnew_id_to_old_id_map;
                std::map<long long, long long> old_id_to_new_id_map;
                for(long long i = 0; i < igraph_vector_int_size(&new_id_to_old_id_vector_map); i ++) {
                ...
                while(!(is_non_trivial_cut || is_well_connected)) {
                    ...
                    std::vector<long long> in_partition_candidate = mcc.GetInPartition();
                    std::vector<long long> out_partition_candidate = mcc.GetOutPartition();
                    ...
                    if(is_well_connected || is_non_trivial_cut) {
                        ...
                        if(in_partition_candidate.size() == 1) {
                            ...
                            for(long long i = 0; i < igraph_vector_int_size(&newnew_id_to_new_id_map); i ++) {
                                long long newnew_id = i;
                                long long new_id = VECTOR(newnew_id_to_new_id_map)[newnew_id];
                                long long old_id = new_id_to_old_id_map[new_id];
                ...
                if(is_well_connected) {
                    ...
                    current_cluster = std::vector<long long>(current_cluster_set.begin(), current_cluster_set.end());
                    {
                    ...
                    if(in_partition.size() > 1) {
                        std::vector<std::vector<long long>> in_clusters = CM::RunClusterOnPartition(&induced_subgraph, algorithm, seed, clustering_parameter, in_partition);
                        for(size_t i = 0; i < in_clusters.size(); i ++) {
                            std::vector<long long> translated_in_clusters;
                    ...
                    if(out_partition.size() > 1) {
                        std::vector<std::vector<long long>> out_clusters = CM::RunClusterOnPartition(&induced_subgraph, algorithm, seed, clustering_parameter, out_partition);
                        for(size_t i = 0; i < out_clusters.size(); i ++) {
                            std::vector<long long> translated_out_clusters;
    ...
    private:
        ...
        static inline std::queue<std::vector<long long>> to_be_mincut_clusters;
        static inline std::mutex to_be_clustered_mutex;
        static inline std::queue<std::vector<long long>> to_be_clustered_clusters;
        static inline std::mutex done_being_clustered_mutex;
        static inline std::queue<std::vector<long long>> done_being_clustered_clusters;
};
